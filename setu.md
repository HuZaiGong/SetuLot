...
| `aspectRatio` |  `string`  |                | 图片长宽比，[详见下文](#aspectRatio)                                                                          |
...
1. 你可以设置为任何[假值](#关于布尔值参数)来得到原始的 i.pximg.net 图片地址
...
- **自动更新作品信息**  
  被获取的作品如果是两年内上传的，且距离上次更新信息超过 90 天，则会被加入更新队列在后台进行信息更新，以防止作品修改或删除造成的原图地址失效。但当次调用以及直到信息更新完毕前，从 API 获取到的信息仍会是旧的信息。